...
Box\<T>是最简单的智能指针：允许在堆上存储数据而不是栈上存储，栈上是指向堆数据的指针，也就是说会把创建的数据保存在堆上。它没有性能开销，也没有其它额外的功能。 它也实现了 Deref trait 和 Drop trait（这两个东西后面具体讲）。
...
有时候，一个值会有多个所有者，为了支持多重引用，Rust 引入了引用计数 Rc (Reference counting)。
...
Rc 是一个只读的引用计数器，我们无法拿到 Rc 结构内部数据的可变引用来修改这个数据。如果需要修改就要用到 RefCell 智能指针。
...
|                    | Rc\<RefCell\<T>> | val.borrow() | val.borrow_mut() |
| 共享所有权(多线程) | Arc\<T>          | &Arc\<T>     | 无法得到可变借用 |
|                    | Arc\<Mutex\<T>>  | val.lock()   | val.lock()       |
|                    | Arc\<RwLock\<T>> | val.read()   | val.write()      |
...
```rust
pub struct Screen {
  // Box用来定义trait对象，这个结构用来表示 Box 里的元素都要实现了 Draw 这个 trait
  // 而这里如果改成泛型来存储就只能存某一种元素类型，不够灵活
  pub components: Vec<Box<dyn Draw>>,
}
